...
@section('contenido')
    ...
    <div class="container-fluid">
        <div class="row">
              <h3 class="display-3 col-4">Exportar Guias</h3>
              ...
              <form action="{{ route('FirmaGuiasFiltro') }}" method="get" class="col-2 row justify-content-center align-items-center">
        ...
        <h3 class="display-3">Exportar Detalle Guias</h3>
        <div class="row">
          <div class="col-md-12">
            {{-- BUSCADOR --}}
            <div class="table-responsive-xl">
                <table id="detalle" class="table table-bordered table-hover dataTable table-sm">
                    ...
                    <tbody>
                       @foreach($detalle_guias_dia as $item)
                        <tr>
                            <td>{{ $item->documentoid }}</td>
                            <td>{{ $item->rutenvia }}</td>
                            <td>{{ $item->numlinea }}</td>
        ...
        <h3 class="display-3">Exportar Referencia Guias</h3>
        <div class="row">
          <div class="col-md-12">
            {{-- BUSCADOR --}}
            <div class="table-responsive-xl">
                <table id="referencia" class="table table-bordered table-hover dataTable table-sm">
                    ...
                    <tbody>
                       @foreach($referencia_guias_dia as $item)
                        <tr>
                            <td>{{ $item->documentoid }}</td>
                            <td>{{ $item->folioref }}</td>
  ...
  $(document).ready(function() {
        ...
        buttons: [
            {
                ...
                header: false,
                title: '',
                text: 'Exportar Encabezado Guias',
            }
    ...
    var table2 = $('#detalle').DataTable( {
        ...
        buttons: [
            {
                extend: 'excel',
                filename: 'GuiasSIIDet',
                header: false,
                title: '',
                text: 'Exportar Detalle Guias',
            }
    ...
    $('#export').click(function () {
      ...
      XLSX.writeFile(workbook, "DocSiiGD.xlsx");
    });
  });
  </script>
